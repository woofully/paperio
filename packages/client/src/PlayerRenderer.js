...
export class PlayerRenderer {
    constructor(scene, color, playerName) {
        // Track death state
        this.isDead = false;
        // INTERPOLATION VARIABLES
        this.targetPosition = new THREE.Vector3();
        this.currentTrailData = [];
        this.isFirstPositionUpdate = true; // Track first update to prevent fly-in
        this.scene = scene;
        this.color = color;
    ...
    tick(dt) {
        ...
        if (this.currentTrailData.length > 0) {
            const headPos = new THREE.Vector3(this.characterMesh.position.x, this.characterMesh.position.y, 0);
    ...
    dispose() {
        ...
        this.territoryMesh.material.dispose();
